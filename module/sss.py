    ...
    def combine(self, shares):
        ...
        for index, share in enumerate(shares):
            ...
            for i in range(0, count):
                ...
                secrets[index].append([self.util.from_base64(
                    cshare[0:44]), self.util.from_base64(cshare[44:88])])
            ...
            for share_index, share in enumerate(secrets):
                ...
                for product_index, product in enumerate(secrets):
                    if product_index != share_index:
                        ...
                        denominator = (
                            denominator * (origin - current)) % self.util.prime

                working = (
                    (originy * numerator * self.util.mod_inverse(denominator)) + self.util.prime)
                secret[part_index] = (
                    secret[part_index] + working) % self.util.prime
